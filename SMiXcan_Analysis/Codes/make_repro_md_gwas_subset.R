...
md_full_dir <- if (length(args) >= 1) {
  args[[1]]
} else {
  Sys.getenv("MIXCAN_MD_FULL_DIR", unset = file.path(Sys.getenv("MIXCAN_ANALYSIS_DIR", unset = getwd()), "MD_results_2021"))
}
md_full_dir <- normalizePath(md_full_dir, mustWork = TRUE)

repo_data_dir <- normalizePath(file.path(get_script_dir(), "..", "..", "Data"), mustWork = TRUE)
ref_dir <- file.path(repo_data_dir, "1000Genome_Ref")
out_dir <- file.path(repo_data_dir, "MD_results_2021")
...
cat(sprintf("Reading full MD GWAS files from: %s\n", md_full_dir))

for (pheno in c("DA", "NDA", "PMD")) {
  infile <- file.path(md_full_dir, sprintf("%s_MetaResultswInfo_20210609_hg38.txt", pheno))
  outfile <- file.path(out_dir, sprintf("%s_MetaResultswInfo_20210609_hg38_repro_subset.txt", pheno))

  gwas <- fread(infile)
  keep <- marker_keep(
    chr = gwas[["chr"]],
    pos = gwas[["POS_hg38"]],
    allele1 = gwas[["Allele1"]],
    allele2 = gwas[["Allele2"]]
  )

  fwrite(gwas[keep], outfile, sep = "\t")
  cat(sprintf("%s: kept %d of %d rows -> %s\n", pheno, sum(keep), nrow(gwas), outfile))
}
